...
### Fixed
- **Таймаут TXT больше не отменяет вердикт, уже полученный из A-записи.** `check_dnsbl()` спрашивал зону дважды: сначала A (это и есть вердикт блоклиста), следом TXT (человекочитаемое пояснение). Ошибка любого из двух запросов безусловно перезаписывала статус на `CHECK FAILED` — включая случай, когда A уже вернул однозначный ответ. На проде это выглядело так: `zen.spamhaus.org` отвечал `127.0.0.2, 127.0.0.10, 127.0.0.4` за 0.08 c, а TXT к тому же имени упирался в lifetime ровно на 10.1 c, и живая зона объявлялась мёртвой. За сутки — 29 записей `CHECK FAILED`, из них 25 с валидным A-ответом; канареечный триггер «DNSBL zone cannot prove it is alive» висел на alterc.ru, itforprof.ru и itforpro.ru. Опаснее канарейки то же самое на реальных MX-адресах: настоящий `LISTED` превращался в `CHECK FAILED`, и алерт о попадании в чёрный список терялся. Теперь `CHECK FAILED` ставится только когда A-ответа нет. TXT остаётся тем, чем и был, — пояснением: policy-ответ (`127.255.x`) распознаётся по A и без него.
- **Текст ошибки кладётся в `txt` после классификации, а не до неё.** Раньше он попадал туда до вычисления статуса и мог быть прочитан маркерами `_is_policy_txt` («limit», «blocked», «denied»). Видимое поведение поля не изменилось: текст ошибки по-прежнему виден в `txt`, когда настоящего TXT нет.

### Added
- `tests/test_dnsbl_states.py` — регрессия на всё вышеописанное: A + таймаут TXT даёт `LISTED`, таймаут самого A по-прежнему даёт `CHECK FAILED`, `127.255.x` распознаётся без TXT, живой TXT не подменяется текстом ошибки. До правки падает тремя проверками из семи.

### Changed
- `{$TEMPLATE_VERSION}` и `vendor.version` подняты до `0.1.61`. `vendor.version` отставал на девять версий (`0.1.52`) — он не участвует ни в одной проверке, но в экспорте шаблона это единственное место, где видно его происхождение.

### Docs
- **Из `README_EN.md` удалены две мёртвые секции «Python 3 Not Found» и «dnspython Not Installed».** Обе учили `apt install python3-pip` и `pip3 install dnspython` в системный Python — ровно то, что запрещает раздел про venv двумя экранами выше, и что упирается в PEP 668 на любом современном дистрибутиве. В русском README их убрали раньше, английский отстал.
- **В оба README добавлен раздел «зона не подтвердила, что жива».** Он объясняет, что вердикт блоклиста — это ответ `A`, а `TXT` только поясняет его словами, и что `mail.dnsbl.canary.failed` больше нуля теперь означает настоящую мёртвую зону, а не медленный TXT. Там же отмечено, что медленный TXT `zen.spamhaus.org` по-прежнему тратит время прогона — публичные зеркала лимитируют запросы.
- **В примерах ручного запуска `dnsbl_max_ip` стоял `1` при фактическом умолчании `5`.** Тот же разрыв в таблице макросов закрыли в 0.1.60, но три командных примера остались учить старому значению.

### Removed
- `.pytest_cache/` и `.ruff_cache/` добавлены в `.gitignore`: обе директории накапливались рядом с исходниками и не были перечислены.
