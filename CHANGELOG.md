...
Упрощение без изменения поведения плюс правки по итогам ревью девятью рецензентами. Выходной JSON проверен на идентичность: 155 путей совпадают с 0.1.59, оба режима `--simulate` и аварийный конверт сходятся побайтово, рецензент дополнительно прогнал полный перебор входов `build_lld` и `count_dnsbl_not_checked` — расхождений нет.

### Changed
- **Ручной цикл устранения дублей заменён на `dict.fromkeys`.** Тот же порядок первого вхождения, тот же отсев пустых значений, семь строк вместо одной. Комментарий о том, зачем дубли вообще отсеиваются, оставлен: он объясняет, что обнаружение ключует элементы по макросу и повтор имени породил бы две строки на один ключ.
- **Снимок кэша и вычисление своих правок вынесены в `snapshot_cache()` и `cache_changes()`.** Раньше обе формулы жили внутри `main()`, а тест повторял их своей копией — и прошёл бы, даже если бы рабочий код с ней разошёлся. Ровно та причина, по которой в 0.1.56 выносили `count_ptr_checked`. Теперь тест вызывает рабочий код, и понижение глубокой копии до `dict(cache)` роняет проверку (проверено подменой).
- **`json.loads(json.dumps(cache))` заменён на `copy.deepcopy(cache)`.** Кэш приходит из `json.load`, так что для нынешних данных обе записи равнозначны; вторая говорит прямо, что нужна независимая копия, и не притворяется сериализацией.

### Fixed
- **Формула `dnsbl.not_checked` вынесена в `count_dnsbl_not_checked()` с тестами.** Арифметика не изменилась. Знаменатель здесь намеренно шире, чем у `count_ptr_checked`: тот отбрасывает `ERROR`, здесь `ERROR` остаётся, потому что `check_ptr_fcrdns` возвращает его и для исправного IPv4, у которого просто не ответил обратный DNS, — а такой адрес квоту потратил и лежит в `checked_ips`. Рецензент предложил счётчики выровнять; предложение отклонено с расчётом: на 25 адресах с 5 проверенными и 4 ошибками PTR выравнивание даёт 16 вместо 20, то есть занижает разрыв покрытия — ровно та ошибка, ради которой счётчик заведён. Тест теперь падает при попытке выровнять.
- **Описание `count_dnsbl_not_checked` обещало больше, чем считает.** Первая редакция утверждала, что счётчик показывает, «про сколько адресов нельзя сказать, что они не в списках». Это неверно: адрес, к которому обратились, но все зоны ответили `CHECK FAILED`, попадает в `checked_ips` раньше самого запроса и потому вычитается, давая ноль, — вердикта про него нет, а считает этот случай отдельный элемент `mail.dnsbl.check_failed.count`. Формулировка сужена до «сколько адресов квота не пустила к спискам», граница закреплена тестом.

### Docs
- `{$DNSBL_MAX_IP}` в обоих README значился как `1`, тогда как шаблон и скрипт держат `5` начиная с 0.1.53. Добавлено, что `0` означает «без ограничения», а не «не проверять».
- `{$MAIL_DNS_NODATA_SEC}` значился как `1800`, тогда как шаблон держит `3h`. Значение `1800` при часовом опросе оставалось истинным полчаса из каждого часа и заставляло триггер мигать постоянно — это записано в описании макроса, но README продолжал публиковать именно его.
- `{$TEMPLATE_VERSION}` значился как `0.1.52`, отставая на семь версий.
- Добавлен раздел о том, как запускать тесты и линтер. Команды нигде не были записаны, а `tests/test_template_contracts.py` требует PyYAML, которого в рабочем окружении нет намеренно, — то есть запускается только системным интерпретатором, и узнать об этом было неоткуда.

### Security
- **Из репозитория убраны сведения, идентифицирующие наблюдаемые домены и инфраструктуру.** Репозиторий публичный, и в CHANGELOG попали имена четырёх обслуживаемых доменов, имя боевой машины и нейм-сервер провайдера — сами по себе не секрет, но вместе они раскрывают, чью почту здесь наблюдают. Заменены нейтральными описаниями; технический смысл записей сохранён полностью, потому что ценность в уроке, а не в имени домена.
- **`tests/sample_output.json` обезличен.** Эталон был живым прогоном по домену автора и нёс реальные адреса почтового сервера и нейм-серверов, адреса отчётов DMARC с идентификаторами учётной записи стороннего сервиса и ссылку BIMI. Всё это опубликовано и в DNS, но собранное в один файл становится готовой выборкой. Заменено на диапазоны из RFC 5737 и RFC 2606, хэши пересчитаны. Структура и типы всех полей сверены до и после — тест шаблона опирается только на них.

### Note
Не всё найденное исправлено здесь, и два пункта вынесены отдельно, потому что это изменение поведения, а не упрощение. Первый: `classify_address` возвращает `SKIP_IPV6` независимо от `{$CHECK_IPV6}`, поэтому при `=1` адреса AAAA собираются, но их PTR не проверяется, а из знаменателя `not_checked` они исключены — то есть непроверенное читается как «всё покрыто». Ветка `and not check_ipv6` в `check_ptr_fcrdns` при этом недостижима. Сейчас безопасно: на всех хостах макрос равен `0` или пуст (проверено через API), но описание макроса приглашает его включить. Второй: при пустом `{$DNSBL_ZONES}` адрес всё равно попадает в `checked_ips`, разрыв показывает ноль, и «проверок не было» выглядит как «всё чисто». Оба поведения существовали и до этой правки.
